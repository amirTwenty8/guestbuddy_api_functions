...
/**
 * Generate cards for a club card
 * This function is called separately to generate cards in the background
 */
export const generateClubCards = onCall({
  enforceAppCheck: false,
  timeoutSeconds: 540, // 9 minutes
  memory: "1GiB", // More memory for large operations
}, async (request) => {
  try {
    console.log('🚀 generateClubCards STARTED');
    
    const { companyId, cardId, totalCards, cardTitle, userName, existingItems = [] } = request.data;
    
    console.log(`Starting generation of ${totalCards} cards for ${cardId}`);
    ...
    try {
      for (let batchStart = 0; batchStart < totalCards; batchStart += batchSize) {
        ...
        console.log(`Batch ${Math.floor(batchStart / batchSize) + 1}: generating cards ${batchStart + 1}-${batchEnd}`);
        ...
        console.log(`Batch completed: ${generatedCount}/${totalCards} cards generated`);
      ...
      console.log(`✅ Generation completed: ${generatedCount} cards generated for ${cardId}`);
      ...
      console.error(`Error in generation for ${cardId}:`, error);
    ...
    console.error("Error in generateClubCards:", error);
    return {
      success: false,
